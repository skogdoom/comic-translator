...
is wanted. Then the window irritations — 14, 15 and 4.28, each an S, with 4.27
behind them because it is the same corner of the same files even though it is
...
**Already there, asked for and measured.** Rendering a chapter as JPEG, as
loose pages and inside a `.cbz` alike, works today. Measured:
`apply --output chapter.cbz --format jpeg` writes `001-page-001.jpg` and the
rest, real JPEG bytes, and the render window offers the same two choices side
by side.
It earns a regression test wherever `apply` is next opened up, not a
milestone.
