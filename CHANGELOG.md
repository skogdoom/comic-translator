...
- **A sound effect can be lettered over the art.** Edit > Make Sound Effect,
  ...
  `stroke_color`, in the format since version 4 and now drawn: 8% of the
  fitted font size, never under a pixel, with room for it left inside the
...
- **A chapter can be read without translating it.** `comictrans read` opens a
  ...
  right to left, with zoom in the View menu or by a pinch on a trackpad, no
  toolbar, and a slider and page count along the foot of the window while the
  pointer is there. Two at a time opens like a printed comic, the cover alone
  and then facing pairs, and a page wider than tall — a spread scanned as one
  — is shown alone. A chapter file is read where it is and never unpacked, so
  opening one leaves nothing beside it; it goes through the same checks
  `extract` makes, because it is handed the entries `unpack` writes rather
  than opening the archive another way. Pages are decoded ahead on a thread,
  and six at most are held however long the chapter is.
